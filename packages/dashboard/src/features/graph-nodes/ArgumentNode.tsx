...
export function ArgumentNode({ data, selected }: { data: FacetNodeData; selected: boolean }) {
  const highlightRing = data.highlighted ? " ring-2 ring-amber-500/60" : "";
  return (
    <div
      className={`bg-surface-canvas border rounded-md py-2.5 px-4 max-w-[200px] ${selected ? "border-accent-primary shadow-[0_0_12px_rgba(59,130,246,0.2)]" : data.highlighted ? `border-amber-500${highlightRing}` : "border-border-default"} shadow-[0_2px_8px_rgba(0,0,0,0.5)] ${data.dimmed ? "opacity-40" : ""}`}
